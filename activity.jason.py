...
async def get_contributors_gitee(session, platform, repo_name):
    url_repo_contributors = f"https://oss.open-digger.cn/{platform}/{repo_name}/activity.json"
    try:
...
async def process_repositories():
    # 从 Excel 文件读取数据
    repo_data = pd.read_excel(r"D:\Open Rank\data\repo_list_github.xlsx")
    ...
    # 保存为新的 Excel 文件
    repo_data.to_excel("repo_github_activity_M.xlsx", index=False)
    print("Data has been saved to repo_github_activity_M.xlsx")
